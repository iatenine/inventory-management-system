...
const resolvers = {
  Query: {
    user: async (parent, args, context) => {
      ...
      const users = await User.findOne({ _id: args._id }).populate({
        path: "inventories",
        populate: { path: "items" },
      });
    ...
    inventory: async (parent, args, context) => {
      const inventories = await Inventory.findOne({ _id: args._id }).populate(
        "items"
      );
      if (!inventories) {
    ...
    getItem: async (parent, args, context) => {
      const items = await Item.findOne({ _id: args._id });
      if (!items) {
        throw new AuthenticationError(items);
      }
  ...
  Mutation: {
    login: async (parent, { email, username, password }) => {
      ...
      if (!profile) {
        throw new AuthenticationError(
          "No profile with those credentials is found!"
        );
      }
    ...
    deleteUser: async (parent, args, context) => {
      ...
      if (!removeUser) {
        throw new AuthenticationError(
          "Couldn't find user with those credentials!"
        );
      }
    ...
    updateInventory: async (parent, { _id, input }) => {
      ...
      if (!updateInv) {
        throw new AuthenticationError(
          "Something went wrong and we couldn't update your Inventory!"
        );
      }
    ...
    deleteInventory: async (parent, { _id }) => {
      ...
      if (!removeInv) {
        throw new AuthenticationError(
          "Something went wrong and we couldn't remove your Inventory!"
        );
      }
    ...
    updateItem: async (parent, { _id, input }) => {
      ...
      if (!updateItem) {
        throw new AuthenticationError(
          "Something went wrong and we couldn't update your Item!"
        );
      }
